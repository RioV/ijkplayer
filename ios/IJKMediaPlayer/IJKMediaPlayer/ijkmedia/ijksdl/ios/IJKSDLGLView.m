...
#include "ijksdl/ffmpeg/ijksdl_vout_overlay_ffmpeg.c"
...
- (void)displayInternal: (SDL_VoutOverlay *) overlay
{
    if (![self setupRenderer:overlay]) {
        ...
        if (overlay->opaque->linked_frame) {
            CVPixelBufferRef frameREF = [self converCVPixelBufferRefFromAVFrame:overlay->opaque->linked_frame];
